...
![演讲者模式演示](docs/screenshots/presenter-mode-demo.png)
*双屏演讲者视图 — 左侧：当前幻灯片预览；右上：下一页缩略图；右中：计时器；右下：可编辑备注区，自动保存到源文件。*
...
- 📊 **表格编辑** — 增删行列、合并单元格
- 🖼️ **图片处理** — 粘贴图片、替换图片
- ✂️ **图片裁剪** — PPT 风格的图片裁剪，8 向拖拽手柄实时预览，基于 `clip-path` 非破坏性裁剪，不损坏原始图片数据
- 💾 **自动保存** — 首次 `Ctrl+S` 授权后，每次编辑停止 1.5 秒自动覆盖保存源文件
- 🎤 **演讲者模式** — 双屏演讲者视图（全屏后按 `P`）：当前页 + 下一页预览 + 演讲者备注 + 计时器，通过 BroadcastChannel 实时同步翻页
- 📏 **对齐参考线** — 拖拽时自动显示对齐辅助线
- ↩️ **撤销/重做** — 完整的操作历史记录
- 📑 **页面排序** — 支持 PPT 风格的页面重新排序
- 🎨 **Canvas 画板模式** — 类 Figma 自由画板，支持文本、矩形、椭圆、线段、箭头绘制，颜色选择器、SVG/PNG 导出
...
- [x] ✂️ **图片裁剪** — PPT 风格非破坏性裁剪，8 向手柄实时预览
- [x] 💾 **自动保存** — 首次授权后自动覆盖保存源文件
- [x] 🎤 **演讲者模式** — 双屏 PPT 风格演讲者视图，含备注编辑、下一页预览、计时器
- [ ] 🔌 **插件系统** — 可扩展的第三方插件架构
- [ ] 🤝 **协同编辑** — 实时多人编辑支持
